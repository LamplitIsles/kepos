...
9. As a desktop service owner, I want an enabled publisher to create and preserve its own identity on first packaged-app startup, so that desktop setup never requires a repository checkout or CLI initialization command.
10. As a documentation reader, I want distinct Publisher (Desktop) and Publisher (CLI) guides and optional cryptographic verification outside the main onboarding path, so that I see only the workflow relevant to me.
...
- Use the existing Vite website and shared visual language for the primary `/docs/` guide plus one auxiliary `/docs/verify/` reference. This remains static HTML, not a documentation framework, client-side router, content management system, or duplicate application. Further route proliferation is deferred until navigation volume proves it necessary.
...
- The documentation information architecture starts with **Publisher and subscriber**, then exposes distinct **Publisher (Desktop)**, **Publisher (CLI)**, and **Subscriber** entries. The canonical first-success flow is a macOS or Windows packaged desktop publisher paired through **Add device** with an Android or desktop subscriber. Repository checkout, `setup publisher`, explicit state directories, manual public-key/TOML operations, and headless CLI/Nix/container/NUC lifecycle belong only to Publisher (CLI).
...
- Packaged desktop applications create default configuration and enabled role identities idempotently when absent, preserve existing configuration and identities, and use packaged bootstrap endpoints. When `[publisher].enabled = true`, desktop startup creates missing publisher state from the configured display name, allowlist, and services before starting that role; repeated startup validates and reuses the same identity. Android creates and preserves its app-private subscriber identity. Never instruct users to copy secret identity state between devices.
...
- Preserve `releases/latest/download/<artifact>` links and explain that they follow the latest stable GitHub release, not prereleases. The main download section states the unsigned/ad-hoc-signed developer-preview boundary and links once to optional verification. Complete SHA-256 and Minisign steps live only on `/docs/verify/` and do not interrupt the primary onboarding flow.
...
- A graphical service configuration editor, setup wizard, relay support, UDP transport in Kepos, signing/notarization, or changes to release artifacts.
